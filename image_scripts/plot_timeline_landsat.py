...
sat_dates = {'Landsat 1':(np.datetime64('1972-01-01'),np.datetime64('1978-01-21')),
             ...
             'Landsat 7':(np.datetime64('1999-01-01'),np.datetime64('2024-01-19')),
             'Landsat 8':(np.datetime64('2013-01-01'),np.datetime64('2026-12-31')),
             'Landsat 9':(np.datetime64('2021-01-01'),np.datetime64('2026-12-31'))}
...
# format the axes
plt.gca().set_xlim([np.datetime64('1972-02-01'),np.datetime64('2027-01-01')])
plt.gca().set_ylim([np.ceil(len(sat_names)/2),-0.25])
myFmt = mdates.DateFormatter('%Y')
plt.gca().xaxis.set_major_formatter(myFmt)
plt.gca().set_yticks([])
plt.title('Data Acquisition Periods of Landsat Program Satellites')
